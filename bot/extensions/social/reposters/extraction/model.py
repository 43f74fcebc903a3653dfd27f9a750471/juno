...
class HttpHeaders(BaseModel):
    User_Agent: Optional[str] = Field(None, alias="User-Agent", title="User-Agent")  # type: ignore
    Accept: Optional[str] = Field(None, title="Accept")
    Accept_Language: Optional[str] = Field(
        None,
        alias="Accept-Language",
        title="Accept-Language",  # type: ignore
    )
    Sec_Fetch_Mode: Optional[str] = Field(
        None,
        alias="Sec-Fetch-Mode",
        title="Sec-Fetch-Mode",  # type: ignore
    )
...
class Format(BaseModel):
    ...
    quality: Optional[float] = Field(None, title="Quality")
...
class RequestedDownload(BaseModel):
    ...
    def public_url(self) -> str:
        return f"{config.backend.public_url}/c/{self.filepath.split('/', 3)[-1]}"
    ...
    async def delete(self, instant: bool = True) -> Optional[asyncio.Task]:
        if not instant:
            logger.debug(f"Scheduling {self.filepath} for deletion")
            return asyncio.create_task(self.delete_later())

        target = AsyncPath(self.filepath)
        logger.debug(f"Deleting {self.filepath} which is {self.file_size} bytes")
        with suppress(FileNotFoundError):
            if target.parent != AsyncPath("/tmp/juno"):
                async for child in target.parent.iterdir():
                    await child.unlink(missing_ok=True)

                await target.parent.rmdir()
                return

            await target.unlink(missing_ok=True)

    async def delete_later(self) -> None:
...
class Information(BaseModel):
    ...
    resolution: Optional[str] = Field(None, title="Resolution")
    http_headers: Optional[HttpHeaders] = None
    requested_downloads: Optional[list[RequestedDownload]] = Field(
        None, title="Requested Downloads"
    )
