...
class EmbedBuilder:
    ...
    def __call__(self, node: Node) -> Embed:
        try:
            func: Callable[[Node], None] = getattr(self, node.name.replace(".", "_"))
    ...
    def color(self, node: Node) -> None:
        with suppress(ValueError):
            self.embed.color = int(node.value.lstrip("#"), 16)
    ...
    def thumbnail(self, node: Node) -> Embed:
        if node.value.lower() in {"none", "null", "false", ""}:
            return self.embed
    ...
    def footer(self, node: Node) -> None:
        ...
        text = parts[0].strip()
        icon_url = parts[1].strip() if len(parts) >= 2 else None
