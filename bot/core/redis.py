...
GLOBAL_REDIS: Redis = None  # type: ignore
...
class Redis(DefaultRedis):
    ...
    async def hset(
        ...
        return await super().hset(name, key, value)  # type: ignore
    ...
    async def hget(
        ...
        output = await super().hget(name, key)  # type: ignore
        if output:
            if isinstance(output, bytes):
    ...
    async def hgetall(
        ...
        output = await super().hgetall(name)  # type: ignore
        return {k.decode("utf-8"): v.decode("utf-8") for k, v in output.items()}
